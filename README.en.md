...
> **Current release focus (v1.0.4):** this fork currently focuses on two home-network models: the **Vendor Traffic Model** and the **Availability Monitoring Model**. The first answers “which device accessed which vendor, over which protocol, and with how much traffic”; the second answers “whether a network device or service is reachable, how fast it responds, and when it failed”. High-confidence IP-to-domain associations are now included in vendor summaries without rewriting the original records.
