...
- Fixed availability history and recent-incident timestamps: UTC buckets without a `Z` suffix are now parsed as UTC before being rendered in Asia/Shanghai, avoiding an eight-hour offset from the health chart.
- Centralized availability timestamp parsing for ISO timestamps, SQLite UTC timestamps, and values with explicit offsets.
- Corrected the Docker release workflow to publish consistently to `uggugg/home-network-monitor`.

## [1.0.4] - 2026-08-14

### Fixed

- High-confidence IP-to-domain associations now apply consistently to vendor totals, device rankings, trends, protocol statistics, and top endpoint drill-downs.
- Original Unknown rollups remain unchanged; only query-time presentation is reclassified and the association evidence is preserved.
- Recognition-rate semantics are corrected: enriched IP traffic counts toward total recognition, but not domain recognition.

## [1.0.3] - 2026-08-14

### Added

- Unknown candidates can now be manually probed using DNS, HTTPS page metadata, and RDAP/WHOIS clues to generate vendor suggestions.
- Probe results can be confirmed and prefilled into manual vendor rules; the endpoint only accepts current Unknown candidates to prevent arbitrary probing.

## [1.0.1] - 2026-08-13

### Fixed

- The in-app logo now uses the versioned `/logo-hnm-v1.png` asset path so upgrades cannot reuse a stale `/logo.png` entry from Next Image, browser, or PWA caches.
- Sidebar, About dialog, mobile header, favicon, and shortcut icon references now consistently use the new HNM logo.
